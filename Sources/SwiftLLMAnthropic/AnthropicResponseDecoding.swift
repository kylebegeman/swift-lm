...
extension AnthropicClient {
  public func respond(to request: LLMRequest) async throws -> LLMResponse {
    ...
    return try payload.llmResponse(metadata: metadata(for: request))
  }
  ...
  static func failure(statusCode: Int?, body: Data) -> LLMClientError {
...
/// Builds `LLMResponse` values identically for the non-streaming and streaming paths.
  ...
  static func response(
    ...
    let message = LLMMessage.assistant(
      ...
      providerContent: hasThinkingBlocks
        ? LLMProviderContent(providerKind: .anthropic, payload: .array(rawBlocks))
        : nil
    )
    ...
    return LLMResponse(
      ...
      message: message,
      toolCalls: toolCalls,
      finishReason: stopReason.map(LLMFinishReason.init(anthropicStopReason:)),
