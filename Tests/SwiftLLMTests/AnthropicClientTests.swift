...
struct AnthropicClientTests {
  ...
  func anthropicClientAppliesModelFamilyRules() async throws {
    ...
    await #expect(throws: LLMClientError.self) {
      _ = try await opus.respond(to: LLMRequest(messages: [.user("Hi")], parameters: .deterministic))
    }
    await #expect(throws: LLMClientError.self) {
      _ = try await haiku.respond(
        to: LLMRequest(messages: [.user("Hi")], parameters: LLMGenerationParameters(reasoningEffort: .low))
      )
    }
  }
  ...
  func anthropicClientEncodesReasoningStructuredOutputAndCaching() async throws {
    ...
    let response = try await client.respond(
      to: LLMRequest(
        ...
        tools: [LLMToolDefinition(name: "lookup", description: "Look up.", inputSchema: ["type": "object", "additionalProperties": false])],
        parameters: LLMGenerationParameters(reasoningEffort: .medium)
      )
    )
  ...
  func anthropicClientOmitsThinkingDisplayForModelsThatPredateIt() async throws {
    ...
    _ = try await client.respond(
      to: LLMRequest(
        messages: [.user("Think.")],
        parameters: LLMGenerationParameters(temperature: 0.5, reasoningEffort: .low)
      )
    )
    let body = try #require(await capture.value()).jsonObject()
  ...
  func anthropicClientReplaysThinkingBlocksWithToolResults() async throws {
    ...
    let assistantTurn = LLMMessage.assistant(
      "",
      toolCalls: [LLMToolCall(id: "toolu_1", name: "lookup_note", argumentsJSON: #"{"id":"1"}"#)],
      providerContent: LLMProviderContent(providerKind: .anthropic, payload: [thinkingBlock, toolUseBlock])
    )
  ...
  func anthropicClientClassifiesErrorsAndRefusals() async throws {
    func reason(status: Int, body: String) async -> LLMClientErrorReason? {
      let client = AnthropicClient(
        apiKey: "test-key",
        model: "claude-test",
        transport: AnthropicHTTPTransport { _ in
      ...
      do {
        _ = try await client.respond(to: LLMRequest(messages: [.user("Hi")]))
        return nil
  ...
  func anthropicClientStreamsThroughInjectedTransport() async throws {
    ...
    for try await event in client.stream(
      to: LLMRequest(
        ...
        parameters: LLMGenerationParameters(reasoningEffort: .low),
  ...
  func anthropicClientDropsTruncatedToolCalls() async throws {
    ...
    var events: [LLMStreamEvent] = []
    for try await event in client.stream(to: LLMRequest(messages: [.user("Lookup.")])) {
      events.append(event)
    }
  ...
  func anthropicClientThrowsStreamErrorEvents() async throws {
    func reason(lines: [String]) async -> LLMClientErrorReason? {
      let client = AnthropicClient(
        apiKey: "test-key",
        model: "claude-test",
        transport: AnthropicHTTPTransport(
