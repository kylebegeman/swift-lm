...
struct OpenAIClientTests {
  ...
  func openAIClientRejectsStopSequencesAndSamplingOnReasoningModels() async throws {
    ...
    await #expect(throws: LLMClientError.self) {
      _ = try await reasoningClient.respond(
        to: LLMRequest(messages: [.user("Hi")], parameters: .deterministic)
      )
    }
    await #expect(throws: LLMClientError.self) {
      _ = try await reasoningClient.respond(
        to: LLMRequest(
          messages: [.user("Hi")],
          parameters: LLMGenerationParameters(stopSequences: ["END"])
        )
      )
    }
  }
  ...
  func openAIClientEncodesReasoningEffortAndReplaysReasoningItems() async throws {
    ...
    let response = try await client.respond(
      to: LLMRequest(
        ...
        tools: [LLMToolDefinition(name: "lookup_note", description: "Look up a note.", inputSchema: ["type": "object"])],
  ...
  func openAIClientSurfacesRefusalsAndIncompleteResponses() async throws {
    ...
    do {
      _ = try await refusingClient.respond(to: LLMRequest(messages: [.user("Do something unsafe.")]))
    ...
    let truncated = try await truncatedClient.respond(to: LLMRequest(messages: [.user("Long answer.")]))
  ...
  func openAIClientClassifiesHTTPErrors() async throws {
    ...
    func reason(status: Int, body: String) async -> LLMClientErrorReason? {
      do {
        _ = try await client(status: status, body: body).respond(to: LLMRequest(messages: [.user("Hi")]))
        return nil
      } catch let error as LLMClientError {
        return error.reason
      } catch {
  ...
  func openAIClientStreamsIncompleteResponsesAsTruncated() async throws {
    ...
    var events: [LLMStreamEvent] = []
    for try await event in client.stream(to: LLMRequest(messages: [.user("Stream.")])) {
      events.append(event)
    }
  ...
  func openAIClientThrowsStreamErrorEvents() async throws {
    ...
    func reason(lines: [String]) async -> LLMClientErrorReason? {
      do {
        for try await _ in client(lines: lines).stream(to: LLMRequest(messages: [.user("Stream.")])) {}
        return nil
      } catch let error as LLMClientError {
        return error.reason
      } catch {
