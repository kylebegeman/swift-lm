...
SwiftLM now has one client surface for multiple model backends:

- Apple Foundation Models through `SwiftLMFoundationModels`
- OpenAI through `SwiftLMOpenAI`
- Anthropic through `SwiftLMAnthropic`
- deterministic local/test clients through `SwiftLM`
...
Provider-neutral usage starts with `LMClient`:
...
Capability negotiation is intentionally explicit. Adapters publish support for tools, tool results, response formats, sampling options, stop sequences, native JSON schema support, streaming, and context windows where known. `LMRouter` uses those capabilities before dispatch so a request that requires tools can skip a local adapter that cannot honor them.
...
`OpenAIClient` translates `LMRequest` into the OpenAI Responses API:
...
- response and streaming transports are injectable; the live transport uses a ten-minute timeout and normalizes `URLError` into `LMClientError`
...
`AnthropicClient` translates `LMRequest` into the Anthropic Messages API:
...
- assistant tool calls and tool-result messages become native `tool_use` and `tool_result` content blocks; thinking blocks captured on a previous response replay verbatim through `LMMessage.providerContent`
...
`LMRouter` lets an app express a fallback ladder:

```swift
let client = LMRouter(
  primary: AnyLMClient(FoundationModelClient.live),
  fallbacks: [
...
The default policy does not retry bad requests, authentication failures, guardrail/refusal failures, decoding bugs, validation failures, cancellations, or unknown provider errors. Apps can opt into `.always`, `.never`, or a custom `LMRouterFallbackPolicy`.

Streaming uses `LMStreamFallbackMode.beforeFirstOutput` by default. If a provider fails before yielding text, tool calls, or a completion event, the router can continue with a fallback provider. Once output starts, the stream fails rather than silently splicing two providers into one partial response.
...
`LMRouter(runReceiptHandler:)`. Receipts intentionally store request shape and
provider metadata, not prompt text, context text, tool arguments, or response
text.

## Endpoint Registry

`LMEndpointRegistry` lets apps register already-created clients under stable
IDs and then build routers from a routing plan:

```swift
let registry = LMEndpointRegistry(
  endpoints: [
    LMEndpoint(id: "local", client: AnyLMClient(FoundationModelClient.live), priority: 10),
    LMEndpoint(id: "openai", client: openAI, priority: 20),
    LMEndpoint(id: "anthropic", client: anthropic, priority: 30),
  ]
)
...
routing and rejected when requested explicitly. `LMRoutingPlan` can pin a
primary endpoint, choose explicit fallback IDs, opt into automatic fallbacks, or
attach the same run-receipt handler that `LMRouter` accepts.
...
let registry = LMEndpointRegistry(
  endpoints: [
    LMEndpoint(id: "on-device", client: AnyLMClient(local), priority: 10),
    LMEndpoint(id: "private-cloud", client: AnyLMClient(local.targeting(.privateCloudCompute)), priority: 20),
  ]
)
```

The registry intentionally stores `AnyLMClient` values, endpoint IDs, priority,
...
SwiftLM accepts API keys at initialization time and does not persist them or define a credential storage policy.
...
That keeps SwiftLM useful for private projects without baking in a public-library security stance too early.
